                ...
                if (that[0] !== undefined) {
                    ...
                    if (type == 'parent') {
                        ...
                        if (el === undefined) {
                            ...
                            var getA = p.getAttribute(_X.XReturnClassOrId(el));
                            var repstr = el.replace(/[.#]/g, '');
                            while ((' ' + getA + ' ').indexOf(' ' + repstr + ' ') < 0 && p !== _X('html')[0]) {
                                p = p.parentNode;
                                getA = p.getAttribute(_X.XReturnClassOrId(el));
                                self[0] = p;
                            }
                        }
                    //children    
                        ...
                        //search by ','
                            ...
                            var elem = type.replace(/\s/g, '').split(',');
                            _X.Xeach(elem, function(k, v) {
                                temp.push( _X.XSearchChildren({a: that, s: v})[0] );
                            });
                        //class, id, tag
                        } else {
                            temp = _X.XSearchChildren({a: allC, s: type});
                        }
                        _X.Xeach(temp, function(k, v) {
                            self[k] = v;
                        });                        
